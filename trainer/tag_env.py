...
class VecTagEnv:
    ...
    def __init__(self, num_envs: int, config: Optional[TagEnvConfig] = None):
        ...
        # Observation dimension: position(2) + velocity(2) + relative(4) + roles(2) + forward(2) + rays(72)
        self.obs_dim = 12 + self.cfg.num_rays * 2
    ...
    def reset(self, env_ids: Optional[np.ndarray] = None) -> Dict[str, np.ndarray]:
        ...
        for i, eid in enumerate(env_ids):
            # Place agents with guaranteed separation
    ...
    def _cast_rays(self, eid: int, agent_idx: int) -> np.ndarray:
        """Cast vision rays for an agent, returns [dist, is_agent] pairs."""
        ...
        for i, angle in enumerate(angles):
            ...
            result[i * 2] = min(dist / self.cfg.ray_max_dist, 1.0)
            result[i * 2 + 1] = is_agent
