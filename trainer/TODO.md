...
- [x] Replace the placeholder L2 "PPO" update with a clipped-surrogate PPO loss so learning matches PRD expectations. *(trainer/ppo.py now implements clipped PPO with entropy/KL metrics.)*
- [x] Split seeker/hider policies (`policy_seeker.pt`, `policy_hider.pt`). *(saves/loads per-role policies; debug bundle copies new files.)*
- [x] Expand metrics logging (reward mean/sum, episode length) and ensure plotting scripts stay in sync. *(`trainer/plot_metrics.py` renders new charts.)*
- [x] Harden reset, timeout, and tag-event handling (avoid stale buffers).
...
- [ ] Guard the monitoring pipeline against metrics schema drift by validating required columns (reward, win flags, PPO diagnostics) before charting and surfacing actionable errors. *(trainer/monitor_runs.py)*
