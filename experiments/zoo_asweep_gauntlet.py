#!/usr/bin/env python3
"""
Compute Forgetting Regret (FR) for each zoo_asweep run via checkpoint gauntlet.

For each run (seed 0): subsample ~12 checkpoints, evaluate all seeker-vs-hider
pairs (20 episodes each), compute FR from the resulting win matrix.

Usage:
  # Single run by task ID (for SLURM array)
  python experiments/zoo_asweep_gauntlet.py --task-id $SLURM_ARRAY_TASK_ID

  # Dry-run to see task list
  python experiments/zoo_asweep_gauntlet.py --list

  # Aggregate results after all tasks finish
  python experiments/zoo_asweep_gauntlet.py --aggregate
"""
import argparse
import csv
import itertools
...
# ── Sweep axes (same as zoo_asweep_tasks.py) ──────────────────────
...
MAX_STEPS = 200
LAYOUT = "four_corners"
BASE_DIR = Path("experiments/results/zoo_asweep")
...
def main():
    parser = argparse.ArgumentParser(description="Zoo A-sweep gauntlet FR computation")
