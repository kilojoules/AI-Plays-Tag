#!/usr/bin/env python3
"""
Generate the task table for the zoo A-sweep and run a single task by index.
...
Usage:
  ...
  python experiments/zoo_asweep_tasks.py --list

  # Run task i (called from SLURM array)
  python experiments/zoo_asweep_tasks.py --task-id $SLURM_ARRAY_TASK_ID

  # Dry-run task i
  python experiments/zoo_asweep_tasks.py --task-id 0 --dry-run
"""
import argparse
import itertools
import os
...
def main():
    parser = argparse.ArgumentParser(description="Zoo A-sweep task launcher")
