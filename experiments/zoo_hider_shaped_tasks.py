#!/usr/bin/env python3
"""
Generate task table for zoo A-sweep with hider distance-change shaping.

Reduced sweep to validate the new reward before full-scale runs:
...
Usage:
  python experiments/zoo_hider_shaped_tasks.py --list
  python experiments/zoo_hider_shaped_tasks.py --task-id $SLURM_ARRAY_TASK_ID
  python experiments/zoo_hider_shaped_tasks.py --count
"""
import argparse
import itertools
import subprocess
...
# New hider distance-change reward
HIDER_DIST_REWARD = 0.14
HIDER_ABS_DIST_REWARD = 0.1

TIMESTEPS = 10_000_000
LAYOUT = "four_corners"
OUTPUT_BASE = "experiments/results/zoo_hider_shaped"
...
def run_task(task, dry_run=False):
    ...
    cmd = [
        ...
        "--layout", LAYOUT,
        "--seed", str(task["seed"]),
        "--hider-dist-reward", str(HIDER_DIST_REWARD),
        "--hider-abs-dist-reward", str(HIDER_ABS_DIST_REWARD),
    ...
    print(f"  hider_dist_reward={HIDER_DIST_REWARD}  hider_abs_dist_reward={HIDER_ABS_DIST_REWARD}")
...
def main():
    parser = argparse.ArgumentParser(description="Zoo hider-shaped sweep task launcher")
